...
const log = logging_1.LoggerProvider.get('StartCommand');
...
async function handleStart(ctx) {
    ...
    try {
        if (isGroup) {
            ...
            await ctx.reply(`سلام ${user?.first_name ?? 'دوست عزیز'}! 🌹\n\n` +
                'من ربات هوشمند ریز بات هستم. ' +
                'از منوی زیر برای مدیریت گروه استفاده کنید.', { reply_markup: keyboard, parse_mode: 'Markdown' });
        }
        else {
            const keyboard = {
                inline_keyboard: [
            ...
            await ctx.reply('🎉 به ریز بات خوش آمدید!\n\n' +
                ...
                'از منوی زیر برای شروع استفاده کنید.', { reply_markup: keyboard });
        }
    }
    catch (error) {
